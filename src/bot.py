...
from .goals import GoalManager, GoalCategory, DifficultyLevel
...
The user can now start fresh with default settings."""
        ...
        # Goal-related callback handlers
        elif data == "add_goal":
            await self._show_goal_categories(query, user_id)
            
        elif data.startswith("goal_category_"):
            category = data.split("_", 2)[2]
            await self._show_goal_templates(query, user_id, category)
            
        elif data.startswith("goal_template_"):
            template_id = data.split("_", 2)[2]
            await self._create_goal_from_template(query, user_id, template_id)
            
        elif data == "goal_custom":
            await self._show_custom_goal_form(query, user_id)
            
        elif data == "view_goals":
            await self._show_user_goals(query, user_id)
            
        elif data.startswith("goal_detail_"):
            goal_id = int(data.split("_")[2])
            await self._show_goal_details(query, user_id, goal_id)
            
        elif data.startswith("goal_checkin_"):
            goal_id = int(data.split("_")[2])
            await self._goal_checkin(query, user_id, goal_id)
            
        elif data.startswith("goal_complete_"):
            goal_id = int(data.split("_")[2])
            await self._complete_goal(query, user_id, goal_id)
            
        elif data.startswith("goal_delete_"):
            goal_id = int(data.split("_")[2])
            await self._delete_goal_confirm(query, user_id, goal_id)
            
        elif data.startswith("goal_delete_confirm_"):
            goal_id = int(data.split("_")[3])
            await self._delete_goal(query, user_id, goal_id)
            
        elif data == "daily_checkin":
            await self._show_daily_checkin(query, user_id)
...
Are you sure you want to proceed?"""
    ...
    async def _show_goal_categories(self, query, user_id: int):
    ...
    async def _show_goal_templates(self, query, user_id: int, category: str):
    ...
    async def _create_goal_from_template(self, query, user_id: int, template_id: str):
...
Good luck with your new goal! 🌟"""
    ...
    async def _show_custom_goal_form(self, query, user_id: int):
...
You can then add more details."""
    ...
    async def _show_user_goals(self, query, user_id: int):
...
Goals help you stay focused and improve your mental health. Create your first goal!"""
    ...
    async def _show_goal_details(self, query, user_id: int, goal_id: int):
...
What would you like to do?"""
    ...
    async def _goal_checkin(self, query, user_id: int, goal_id: int):
...
You're making great progress! Keep going! 💪"""
    ...
    async def _complete_goal(self, query, user_id: int, goal_id: int):
        """Complete a goal"""
        user_settings = self.db.get_user_settings(user_id)
        language = user_settings.get('language', 'de') if user_settings else 'de'
...
Would you like to set a new goal?"""
    ...
    async def _delete_goal_confirm(self, query, user_id: int, goal_id: int):
...
**This action cannot be undone!**"""
    ...
    async def _delete_goal(self, query, user_id: int, goal_id: int):
        """Delete a goal"""
        user_settings = self.db.get_user_settings(user_id)
        language = user_settings.get('language', 'de') if user_settings else 'de'
    ...
    async def _show_daily_checkin(self, query, user_id: int):
